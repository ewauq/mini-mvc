...
class Debugger
{
    // Types des messages du Debugger.
    const STRING = 0;
    const PDO_QUERY = 1;
    ...
    /**
     * Stocke les messages du Debugger dans self::$output.
     * @param  string  $message  Le message à stocker.
     * @param  int     $type     Le type du message.
     */
    public static function send($context)
    {
        ...
        {
            case self::PDO_QUERY:
                $message = '<label class="pdo-query">PDO Query</label>' . Debugger::colorizeSQL($context['message']);
                break;

            case self::PDO_EXCEPTION:
                $message =  '<label class="pdo-exception">PDO Exception</label>' .
                            $context['message'];
                break;

            case self::EXCEPTION:
                $message =  '<label class="exception">Exception</label>' .
                            '[' . $context['__CLASS__']. '.' . $context['__FUNCTION__'] .
                            '()] >> ' .
                            $context['message'];
                break;

            default:
                $message = '<label class="message">Message</label>' . $context['message'];
                break;
        }

        array_push(self::$output, $message);
    }

    /**
     * Retourne les messages stockés dans le Debugger.
     * @return  array  Les messages enregistrés.
     */
    ...
    /**
     * Affiche le Debugger en passant par la vue.
     * @return  string  Le contenu du Debugger.
     */
    public static function show()
    {
        $debugger_output = self::getOutput();
        include('./views/Debugger.php');
    }
    ...
    private static function colorizeSQL($str)
    {
        ...
        // On éclate la chaine de caractères pour traiter chaque partie l'une après l'autre.
        $exploded_str = explode(' ', $str);

        foreach($exploded_str as $key => $value)
        {
            if(in_array($value, $sql_keywords)) $exploded_str[$key] = str_replace($value, '<span class="sql-keyword">' . $value .  '</span>', $exploded_str[$key]);
            if(in_array($value, $sql_operators)) $exploded_str[$key] = str_replace($value, '<span class="sql-operator">' . $value .  '</span>', $exploded_str[$key]);

        }

        $str = join(' ', $exploded_str);
